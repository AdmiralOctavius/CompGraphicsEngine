...
void BaseGame::OnResize()
{
	...
	//Render Target is which texture in memory the next draw command(s) draw to
	...
	//Depth Stencil State represents 2 different things that share 1 buffer
	//Stencil Buffer - Used to cut out certain parts of the screen so we don't draw to them
	//Depth buffer - stores the 'depth' of each pixel (distance from the camera)
